...
class SameSizeCatEmbeddings(nn.Module):
    def __init__(
        ...
        self.n_tokens = sum([ei for ei in categorical_dims])
        # self.column_idx = column_idx
        # self.embed_input = embed_input
        ...
        if self.categorical_dims:
            self.embed_layers_names = {
                e: e for e in self.categorical_features_idx
            }

        categorical_cols = categorical_features_idx
        self.cat_idx = categorical_features_idx
        ...
        if self.shared_embed:
            self.embed: Union[nn.ModuleDict, nn.Embedding] = nn.ModuleDict(
                {
                    "emb_layer_"
                    + str(self.embed_layers_names[col]): SharedEmbeddings(
                    ...
                    for col, val in zip(self.categorical_features_idx, self.categorical_dims)
                }
            )
        else:
            n_tokens = sum([ei for ei in categorical_dims])
    ...
    def forward(self, X: Tensor) -> Tensor:
        if self.shared_embed:
            cat_embed = [
                self.embed["emb_layer_" + str(self.embed_layers_names[col])](  # type: ignore[index]
                    X[:, col].long()
                ).unsqueeze(
                    1
                )
                for col in self.categorical_features_idx
            ]
...
class SameSizeCatAndContEmbeddings(nn.Module):
    def __init__(
        ...
        # Continuous
        if continuous_features_idx:
            self.cont_idx = continuous_features_idx  # [column_idx[col] for col in continuous_cols]
            if cont_norm_layer == "layernorm":
                self.cont_norm: NormLayers = nn.LayerNorm(len(continuous_features_idx))
            elif cont_norm_layer == "batchnorm":
                self.cont_norm = nn.BatchNorm1d(len(continuous_features_idx))
            else:
                self.cont_norm = nn.Identity()
            if self.embed_continuous:
                self.cont_embed = ContEmbeddings(
                    len(continuous_features_idx),
                    embed_dim,
                    cont_embed_dropout,
                    use_cont_bias,
                )
    ...
    def forward(self, X: Tensor) -> Tuple[Tensor, Any]:
        ...
        if self.continuous_features_idx:
            x_cont = self.cont_norm((X[:, self.cont_idx].float()))
            if self.embed_continuous:
                x_cont = self.cont_embed(x_cont)
        else:
