...
from tab_benchmark.dnns.utils.external.pytorch_widedeep.tabular.transformers.tab_transformer import TabTransformer_
import torch
from tab_benchmark.dnns.datasets import TabularDataset


class TabTransformer(TabTransformer_, BaseArchitecture):
    """TabTransformer architecture. Wrapper around TabTransformer architecture developed in the project
    pytorch_widedeep.
        ...
        if mlp_hidden_dims is None:
            if with_cls_token:
                if embed_continuous:
                    ...
                    mlp_input_dim = embedding_dim + len(continuous_features_idx)
            else:
                if embed_continuous:
