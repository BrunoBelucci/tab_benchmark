...
class BaseExperiment:
    ...
    def unpack_parser(self):
        ...
        if isinstance(dask_job_extra_directives, str):
            ...
            args = shlex.split(dask_job_extra_directives)
            dask_job_extra_directives = [
                f"{args[i]} {args[i + 1]}" if i + 1 < len(args) and not args[i + 1].startswith('-') else args[i]
                for i in range(len(args)) if args[i].startswith('-')
            ]
        self.dask_job_extra_directives = dask_job_extra_directives
