...
class HPOExperiment(BaseExperiment):
    ...
    def run_openml_combination(self, seed_model, task_id, task_repeat, task_sample, task_fold):
        ...
        run_args = dict(model_nickname=self.model_nickname, seed_model=seed_model, task_id=task_id, task_repeat=task_repeat,
                        ...
                        timeout_trial=self.timeout_trial, n_jobs=self.n_jobs)
        if self.check_if_exists:
            if check_if_exists_mlflow(self.experiment_name, **run_args):
        ...
        with mlflow.start_run(run_name=run_name) as run:
            ...
            result = run_openml_combination_hpo(search_algorithm_str=self.search_algorithm, n_trials=self.n_trials,
                                                ...
                                                model_nickname=self.model_nickname, seed_model=seed_model,
                                                n_jobs=self.n_jobs,
                                                task_id=task_id, task_repeat=task_repeat, task_sample=task_sample,
                                                task_fold=task_fold, mlflow_tracking_uri=self.mlflow_tracking_uri,
                                                experiment_name=self.experiment_name, parent_run_uuid=run_uuid,
    ...
    def run_own_combination(self, seed_model, dataset_name_or_id, seed_dataset, fold):
        mlflow.set_tracking_uri(self.mlflow_tracking_uri)
        mlflow.set_experiment(self.experiment_name)
        run_args = dict(model_nickname=self.model_nickname, seed_model=seed_model,
                        dataset_name_or_id=dataset_name_or_id, seed_dataset=seed_dataset, fold=fold,
                        resample_strategy=self.resample_strategy, n_folds=self.k_folds,
                        pct_test=self.pct_test, search_algorithm=self.search_algorithm, n_trials=self.n_trials,
        ...
        with mlflow.start_run(run_name=run_name) as run:
            ...
            result = run_own_combination_hpo(search_algorithm_str=self.search_algorithm, n_trials=self.n_trials,
                                             ...
                                             pct_test=self.pct_test, mlflow_tracking_uri=self.mlflow_tracking_uri,
                                             experiment_name=self.experiment_name, parent_run_uuid=run.info.run_uuid,
                                             models_dict=self.models_dict)
            best_result = result.get_best_result()
            mlflow.log_params(best_result.metrics['config']['model_params'])
            for metric_name, metric_value in best_result.metrics.items():
